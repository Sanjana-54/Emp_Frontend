...
function ListOfEmps() {
  ...
  const delEmpById= async (id)=>{
    let res = await axios.delete(`https://emp-backend-nvs8.onrender.com/emp-api/employees/${id}`);
      if (res.status === 200) {
      
         let updatedRes = await axios.get("https://emp-backend-nvs8.onrender.com/emp-api/employees");
...
// GET ALL EMPS
  ...
  useEffect(() => {
    async function getEmps() {
      let res = await axios.get("https://emp-backend-nvs8.onrender.com/emp-api/employees");
      if (res.status === 200) {
        let resObj = res.data
